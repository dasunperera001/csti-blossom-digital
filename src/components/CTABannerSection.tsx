      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-8 text-center">
        ...
        >
          ...
          >
            ...
            <motion.div 
              className="flex flex-wrap justify-center gap-6 text-primary-foreground/60 text-sm"
              initial={{ opacity: 0 }}
              whileInView={{ opacity: 1 }}
              transition={{ duration: 0.5, delay: 1.8, ease: "easeOut" }}
              viewport={{ once: true }}
            >
              {["✓ Free Initial Consultation", "✓ Customized Solutions", "✓ Proven Results", "✓ Local Expertise"].map((item, index) => (
                <motion.span 
                  key={index}
                  initial={{ opacity: 0, x: -10 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.3, delay: 2.0 + index * 0.1, ease: "easeOut" }}
                  viewport={{ once: true }}
                >
                  {item}
                </motion.span>
